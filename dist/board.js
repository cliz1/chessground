import { pos2key, key2pos, opposite, distanceSq, allPos, computeSquareCenter, queenDir, knightDir, championDir, princessDir, amazonDir, samePos, snareDir, wizardDir, archerDir } from './util.js';
...
function tryAutoCastle(state, orig, dest) {
    ...
    if (!rook || rook.color !== king.color || (rook.role !== 'rook' && rook.role !== 'champion'))
        return false;
...
export function baseMove(state, orig, dest) {
    ...
    if (origPiece.role === 'archer' &&
        ...
        state.animation.skipAnims = new Set([orig]);
...
export function getSnappedKeyAtDomPos(orig, pos, asWhite, bounds) {
    ...
    const validSnapPos = allPos.filter(pos2 => samePos(origPos, pos2) ||
        ...
        archerDir(origPos[0], origPos[1], pos2[0], pos2[1]));
