...
export function baseMove(state: HeadlessState, orig: cg.Key, dest: cg.Key): cg.Piece | boolean {
  ...
  if (
    (origPiece.role === 'archer' || origPiece.role === 'centaur') &&
    captured &&
    (() => {
...
export function setSelected(state: HeadlessState, key: cg.Key): void {
  ...
  if (!isInspectable(state, key)) state.inspectable.dests = undefined;
  else state.inspectable.dests = premove(state, key, { ignoreTurn: true });
}
...
function isInspectable(state: HeadlessState, orig: cg.Key): boolean {
  const piece = state.pieces.get(orig);
